...
TEST_F(MapTest, MapGetPath)
{
    ...
    std::vector<position> path1 =
        map.getPath(position({0, 0}), position({1, 1}));
    std::vector<position> path2 =
        map.getPath(position({5, 5}), position({1, 1}));
    std::vector<position> path3 =
        map.getPath(position({2, 6}), position({4, 1}));
...
TEST_F(MapTest, MapGetSurroundings)
{
    ...
    EXPECT_TRUE(pos[9].x == 1 && pos[9].y == 4);
}
